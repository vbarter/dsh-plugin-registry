...
5. 仅当 `plugins.json` 有变化时提交：`chore: sync discovered dsh plugins`
...
**Local.** `node scripts/discover.mjs` (set `GITHUB_TOKEN` if you can). Serve with any static server. Locale: `中文` / `EN`, persisted as `dsh-registry.locale`, override with `?lang=`.
